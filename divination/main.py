            ...
            if "transformed_hexagram" in reading:
                ...
                if isinstance(reading["transformed_hexagram"], dict):
                    reading["transformed_hexagram"]["trigrams"] = {
                        "upper": {"name": yarrow.get_trigram_name(trans_upper_value), "value": trans_upper_value},
                        "lower": {"name": yarrow.get_trigram_name(trans_lower_value), "value": trans_lower_value},
                    }
...
async def generate_reading(request: ReadingRequest) -> JSONResponse:
    ...
    try:
        ...
        if request.mode in [DivinationMethod.COIN, DivinationMethod.COINS]:
            ...
            return JSONResponse(
                status_code=400,
                content={"detail": f"Invalid divination method: {request.mode}. Must be 'yarrow' or 'coins' or 'coin'"},
            )
